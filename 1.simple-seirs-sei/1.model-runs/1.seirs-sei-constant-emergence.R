...
constant_emergence <- 0 #if 0, phi = mu*M0
odin::can_compile()
...
params_base <- list(
  ...
  M0 = c(200000, 2000), # Initial mosquito population. Change to change V/H ratio and endemicity

  mu_h = 1 / (70 * 365),  # Human natural death rate (WHO Life Tables; 70y life expectancy)

  sigma_h = 1 / 12,       # Human incubation rate (~12 days incubation; Smith et al. 2012)

  gamma_h = 1 / 60,       # Human recovery rate (~60 days infectious; Griffin et al. 2010)

  omega_h = 1 / 180,      # Human loss of immunity rate (~180 days immune; White et al. 2014)

  beta_hv = 0.3, #prob transmission on host from vector Smith et al 2012
  beta_vh = 0.3,  #prob transmission on vector from host Smith et al 2012

  mu_v = 0.1,              # Mosquito natural death rate (Griffin 2010; Lines 1987)

  sigma_v = 1 / 10,       # Mosquito incubation rate (EIP)  (Gu et al, 2003)


  tau = 100,              # Start time of intervention (days)
  delta_D = c(190000, 1900), #killing 95% of vectors from total pop
  delta_t_vec = c(10, 30, 90) # Duration of interventions
)
...
malaria_model <- odin::odin({
  ...
  phi <- if(constant_emergence == 0) mu_v*M0 else mu_v*M ##redundancy here...
  beta_vh <- user()
  mu_v <- user()
  sigma_v <- user()
  psi <- user()
